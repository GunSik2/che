...
export class CheFactory {
   ...
   */
  constructor($resource: ng.resource.IResourceService, $q: ng.IQService, lodash: _.LoDashStatic, cheProfile: CheProfile) {
    // keep resource
  ...
  _getPageFromResponse(data: any, headersLink: any): any {
    ...
    if (!headersLink) {
      ...
      let profile = this.cheProfile.getProfile();
      if (!this.itemsPerPage || !profile.userId) {
        return {factories: data};
      }
      this.pageInfo.currentPageNumber = this.pageInfo.currentPageNumber ? this.pageInfo.currentPageNumber : 1;
      let link = '/api/factory/find?creator.userId=' + profile.userId + '&maxItems=' + this.itemsPerPage;
  ...
  fetchFactories(maxItems?: number): ng.IPromise<any> {
    this.itemsPerPage = maxItems ? maxItems : 30;
  ...
  _getFactories(queryData: any): ng.IPromise<any> {
    ...
    if (profile.$resolved) {
      if (profile.userId) {
        queryData.userId = profile.userId;
        this._queryFactories(queryData).then((data: any) => {
          deferred.resolve(data);
        }, (error: any) => {
          deferred.reject(error);
        });
      } else {
        deferred.reject({data: {message: 'Error. No user Id.'}});
      }
    } else {
      profile.$promise.then((profile: che.IProfile) => {
        queryData.userId = profile.userId;
        this._queryFactories(queryData).then((data: any) => {
          deferred.resolve(data);
        }, (error: any) => {
  ...
  _queryFactories(queryData: any): ng.IPromise<any> {
    return this.remoteFactoryAPI.getFactories(queryData).$promise.then((data: any) => {
      this._updateFactoriesDetails(data.factories).then((factoriesDetails: any) => {
